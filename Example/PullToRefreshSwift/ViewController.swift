...
class ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.tableView.addPullRefreshHandler(refreshCompletion: { [weak self] in
            // some code
            sleep(1)
            self?.texts.shuffle()
        ...
        self.tableView.addPushRefreshHandler(refreshCompletion: { [weak self] in
            // some code
            sleep(1)
            self?.texts.shuffle()
            self?.tableView.reloadData()
            self?.tableView.stopPushRefreshEver(ever: true)
            })
    }
